...
int fullOverlap(int s1, int e1, int s2, int e2) {

    // one point within another range
    // if ( s1 == e1 && s1 >= s2 && s1 <= e2 ) { return 1; } 
    // if ( s2 == e2 && s2 >= s1 && s2 <= e1 ) { return 1; }

    // one is within another
    if ( s1 >= s2 && e1 <= e2 ) { return 1; }
    if ( s2 >= s1 && e2 <= e1 ) { return 1; }
...
int main() {
    ...
    while (!feof(fptr)) {
        ...
        accumulator += fullOverlap(nums[0], nums[1], nums[2], nums[3]);
